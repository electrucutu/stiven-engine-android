...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		{
			titleText.animation.addByPrefix('idle', "Presiona Enter para comenzar", 24);
	...
	// --- LOGICA DE INTRO ADOLESCENTE RANDOM CORREGIDA ---
	var sickBeats:Int = 0;
	...
	override function beatHit()
	{
		...
		if (!closedState)
		{
			...
			if (FlxG.state.subState != null && (FlxG.state.subState is IntroSubstate)) {
				var intro:Dynamic = FlxG.state.subState;
				try {
					intro.deleteCoolText();
				} catch(e:haxe.Exception) {}
			}
			...
			switch (sickBeats)
			{
				case 1:
					...
					if (FlxG.state.subState != null && (FlxG.state.subState is IntroSubstate)) {
						var intro:Dynamic = FlxG.state.subState;
						intro.createCoolText(['Un adolescente random']);
					}

				case 4: 
					if (FlxG.state.subState != null && (FlxG.state.subState is IntroSubstate)) {
						var intro:Dynamic = FlxG.state.subState;
						intro.addMoreText('haciendo un motor de FNF solo');
					}
				...
				case 11: 
					if (FlxG.state.subState != null && (FlxG.state.subState is IntroSubstate)) {
						var intro:Dynamic = FlxG.state.subState;
						intro.deleteCoolText();
						intro.createCoolText(['STIVEN ENGINE', 'DE ??']);
					}

				case 13: 
					if (FlxG.state.subState != null && (FlxG.state.subState is IntroSubstate)) {
						var intro:Dynamic = FlxG.state.subState;
						intro.addMoreText('STIVEN XDDDD');
					}
